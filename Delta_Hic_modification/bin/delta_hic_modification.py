...
class delta_hic_modification:
	...
	def delta_matrix_obj(self,ax):
		...
		dh.outdir=self.outdir
		dh.sam1=self.sam1
		dh.sam2=self.sam2
		dh.mat1_path=self.mat1_path
		dh.mat2_path=self.mat2_path
		dh.exists_minus_mat=self.exists_minus_mat
	...
	def delta_bds_obj(self,ax):  
		...
		print(debs.bound1_path)
		debs.bound2_path=self.bound2_path
		print('bounds2.....')
		print(debs.bound2_path)
		debs.chr=self.chr
	...
	def pre_delta_lay_out(self):
		...
		'''
		top_grids=self.delta_matrix_grid+self.delta_bound_grids
		grids=top_grids+self.multi_modification_grids+self.gene_express_grid+1
	...
	def draw_all_delta_map(self,fig):
		...
		gridsSub2=gridspec.GridSpecFromSubplotSpec(1,1,
				subplot_spec=G[self.delta_matrix_grid:(self.delta_matrix_grid+self.delta_bound_grids),:],wspace=0.5,hspace=0.7)
